...
attnlrp = Composite({
    ...
    PytorchGELUTanh: rules.IdentityRule,
...
class Gemma2RMSNorm(nn.Module):
    ...
    def forward(self, x):
        output = rules.identity(self.norm_forward, x.float())
...
class Gemma2MLP(nn.Module):
    ...
    def forward(self, x):
        return self.down_proj(lf.mul2(self.act_fn(self.gate_proj(x)), self.up_proj(x)))
...
class Gemma2Attention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = lf.mul2(lf.matmul(query_states, key_states.transpose(2, 3)), self.scaling)

        if self.config.attn_logit_softcapping is not None:
            attn_weights = lf.mul2(attn_weights, 1/self.config.attn_logit_softcapping)
            attn_weights = self.tanh(attn_weights)
            attn_weights = lf.mul2(attn_weights, self.config.attn_logit_softcapping)

        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
            attn_weights = lf.add2(attn_weights, causal_mask.detach())

        # upcast attention to fp32
        attn_weights = lf.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        #attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        attn_output = lf.matmul(attn_weights, value_states)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if isinstance(past_key_values, HybridCache) and attention_mask.ndim == 2:
            ...
            if hasattr(self.lm_head, "module"):
                dtype = self.lm_head.module.weight.dtype
            else:
